...
#描述：登录ftp后ftp下载和上传文件
def ftp(host,loginname,loginpassword,getfilename,putfilename):
    ...
    if index == 0 :
        ...
        if index == 0:
            ...
            print u'正在下载文件...'
            #输入下载文件的命令
            child.sendline('get %s'%getfilename)
...
#描述：登录ftp后下载文件
def ftp_get(host,loginname,loginpassword,getfilename):
    #使用spawn构造一个函数，生成一个spawn类的对象
    child = pexpect.spawn('ftp %s'%host)
    #期望具有提示输入用户名的字符出现
    index = child.expect(["(?i)Name", "(?i)Unknown host", pexpect.EOF, pexpect.TIMEOUT])
...
#描述：登录ftp后ftp上传文件
def ftp_put(host,loginname,loginpassword,putfilename):
    #使用spawn构造一个函数，生成一个spawn类的对象
    child = pexpect.spawn('ftp %s'%host)
    #期望具有提示输入用户名的字符出现
    index = child.expect(["(?i)Name", "(?i)Unknown host", pexpect.EOF, pexpect.TIMEOUT])
...
if __name__ == '__main__':
    ...
    ftp_get(host,loginname,loginpassword,getfilename)
    ftp_put(host,loginname,loginpassword,putfilename)
